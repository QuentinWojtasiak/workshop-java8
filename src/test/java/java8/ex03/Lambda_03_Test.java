...
public class Lambda_03_Test {
    ...
    private void forEach(List<Person> source, PersonProcessor processor) {
       // TOD0
    	for(Person per:source)
    	{
    		processor.process(per);
    	}
    }
    // end::forEach[]
    ...
    public void test_verify_person() throws Exception {
        ...
        PersonProcessor verifyPerson = t->{assertTrue(t.getFirstname().startsWith("first"));
        	assertTrue(t.getLastname().startsWith("last"));
